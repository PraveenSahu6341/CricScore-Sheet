...
void runs_checker(int i, int j, int st[2],int n_st[2], int* pl1, int* pl2,int* pos, int* total,ex extra[2] ,pl b_o[2][11], int batting,int* wicket,char wk[], int fielding, int bowler_no, bt batsmen[2][11], bl bowlers[2][11],int this_over[],int overs,char runs[2][overs][6],char* runs_po[2][overs][13],char wicket_str[11][100],pl t_player[2][11],int over_extra[overs],int k[1])
{
    ...
    {
        printf(" %d.%d    %s to %s,no run", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
    ...
    {  batsmen[batting][*pos].ones++;
       ...
       *total +=  1; this_over[i]+=1;
        ...
        {
            printf(" %d.%d    %s to %s, %c run", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name, *runs_po[batting][i][k[0]]);
            *pos-=1;
    ...
    {
        printf(" %d.%d    %s to %s, %c runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name, *runs_po[batting][i][k[0]]);
    ...
    {
        printf(" %d.%d    %s to %s, FOUR, Down the ground good shot by %s", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name, b_o[batting][*pos].player_name);
    ...
    {
        printf(" %d.%d    %s to %s, HURRAY....!!! its SIX, What a shot by %s", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name, b_o[batting][*pos].player_name);
    ...
    else if (*runs_po[batting][i][k[0]] == 'w' || *runs_po[batting][i][k[0]] == 'W')
    {
        ...
        *wicket=*wicket + 1;
        ...
        printf(" %d.%d    %s to %s, OH!!! WICKET %s has to go to the pavilion", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name, b_o[batting][*pos].player_name);
    ...
    else if (*runs_po[batting][i][k[0]] == 'b' || *runs_po[batting][i][k[0]] == 'B')
    {
       ...
       // batsmen[batting][*pos].ball_faced += 1;
        ...
        { *total +=  1; this_over[i]+=1; 
           ...
           if (batsmen[batting][0].count==0 && (*pos==n_st[batting]))
            {        
                printf(" %d.%d    %s to %s,bye,1 run", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
                *pos-=1;
                 batsmen[batting][0].count+=1;
                *pos=st[batting];
        ...
        {
            printf(" %d.%d    %s to %s,byes,2 runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
            *total +=  2; this_over[i]+=2;  
            extra[batting].b+=2;
             over_extra[i]+=2;
        ...
        {*total +=  3; this_over[i]+=3;
         extra[batting].b+=3;
          over_extra[i]+=3;
            if (batsmen[batting][0].count==0 && (*pos==n_st[batting]))
            {        
                printf(" %d.%d    %s to %s,byes,3 runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
               *pos-=1;
               batsmen[batting][0].count+=1;
                *pos=st[batting];
    ...
    else if (*runs_po[batting][i][k[0]] == 'l' || *runs_po[batting][i][k[0]] == 'L')
    {
        ...
        *total +=  1; this_over[i]+=1; 
        if (*runs_po[batting][i][k[0]] == '1')
        {
            ...
            if (batsmen[batting][0].count==0 && (*pos==n_st[batting]))
            {        
                printf(" %d.%d    %s to %s,leg bye,1 run", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
        ...
        {
            printf(" %d.%d    %s to %s,leg byes,2 runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
            *total +=  2; this_over[i]+=2;  
            extra[batting].lb+=2;
             over_extra[i]+=2;
        ...
        else if (*runs_po[batting][i][k[0]] == '3')
        {
           ...
           if (batsmen[batting][0].count==0 && (*pos==n_st[batting]))
            {        
                printf(" %d.%d    %s to %s,leg byes,3 runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
                *total +=  3; this_over[i]+=3;  
                *pos-=1;
                batsmen[batting][0].count+=1;
                *pos=st[batting];
        ...
        {
            printf(" %d.%d    %s to %s,leg byes,4 runs.\n\tI think bowler would be really dissapointed because of these extra runs", i, j, b_o[fielding][bowler_no - 1].player_name, b_o[batting][*pos].player_name);
            *total +=  4;        
            this_over[i]+=4;
            extra[batting].lb+=4;
             over_extra[i]+=4;
...
int main()
{
    char opener,N_striker, same[2], team[2][35], venue[30], opt, umpire[30], date[15], m_time[10], w_toss[35], l_toss[35], batting_team[35], fielding_team[35],st_runs_scored[4],st_ball_faced[4],nst_runs_scored[4],nst_ball_faced[4],total_runs[2][4],total_wkts[2][4],s_this_over[4],wicket_str[11][100]={},wk[20];
   ...
   matchinfo(date, m_time, venue, opt, umpire, team, t_player, b_o, overs, w_toss, l_toss, item, same);
    ...
    for (i = 0; i < overs; i++)
    {
        ...
        if(count==0)
        {
            printf("\n%s and %s are at the crease.", b_o[batting][0].player_name, b_o[batting][1].player_name);
            printf("\n%s is on the strike.%s will open the attack.\n", b_o[batting][0].player_name, b_o[fielding][bowler_no - 1].player_name);
            count++;
        }
        this_over[i]=0;
        k[0]=0;
